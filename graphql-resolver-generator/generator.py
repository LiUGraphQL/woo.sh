...
def generate(input_file, output_dir, config: dict):
    ...
    data = {'types': [], 'types_by_key': [], 'interfaces': [], 'unions': [], 'typeDelete': [], 'edge_types_to_delete': [], 'edge_types_to_update': [], 'edge_objects': []}
    ...
    for type_name, _type in schema.type_map.items():
        ...
        if is_schema_defined_object_type(_type):
            ...
            for field_name, field_type in _type.fields.items():
                ...
                elif is_schema_defined_object_type(inner_field_type) or is_interface_type(inner_field_type) or is_union_type(inner_field_type):
