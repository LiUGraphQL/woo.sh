 ...
 * for "write" mode.
 *
 * Edited by: Robin Keskisärkkä
 * Comment: The method has been
 */
async function executeFieldsSerially(exeContext, parentType, sourceValue, path, fields) {
  // Build the underlying mutation transaction by iterating all mutation ops. Each op returns null during this
  // first round. The transaction is executed only once, when all ops have been added.
  for(let responseName in fields){
    var fieldNodes = fields[responseName];
    var fieldPath = addPath(path, responseName);
    await resolveField(exeContext, parentType, sourceValue, fieldNodes, fieldPath);
  }

  // The first op executes transaction, then all ops return results via the GraphQL pipeline, which
  // ensures that the correct resolver functions are called for the returned object.
  var results = Object.create(null);
  for(let responseName in fields){
    var fieldNodes = fields[responseName];
    var fieldPath = addPath(path, responseName);
    results[responseName] = await resolveField(exeContext, parentType, sourceValue, fieldNodes, fieldPath);
  }
  return results;
}
